...
export default function Dashboard() {
	...
	useEffect(() => {
		const cargarDatos = async () => {
			try {
				const emps = await empleadoService.obtenerTodos();
				setEmpleados(emps);

				const todosLosRegistros: HealthRecord[] = [];
				const ultimasTomas: HealthRecord[] = [];
				const casosDetectados: EmpleadoEnRiesgo[] = [];

				for (const empleado of emps) {
					try {
						const historial = await saludService.obtenerHistorial(
							empleado.numeroDocumento,
						);
						todosLosRegistros.push(...historial);

						if (historial.length > 0) {
							const ultimoRegistro =
								historial[historial.length - 1];
							ultimasTomas.push(ultimoRegistro);

							const alertas =
								analizarSignosVitales(ultimoRegistro);
							alertas.forEach((alerta) => {
								casosDetectados.push({
									empleado,
									alerta,
									fechaToma:
										ultimoRegistro.fechaToma ||
										'Fecha no registrada',
								});
							});

							if (
								ultimoRegistro.clasificacionImc ===
									'Sobrepeso' ||
								ultimoRegistro.clasificacionImc === 'Obesidad'
							) {
								casosDetectados.push({
									empleado,
									alerta: {
						...
						console.log(`Sin registros para: ${empleado.nombres}`);
					}
				}

				setRegistrosGlobales(todosLosRegistros);
				setRegistrosActuales(ultimasTomas);
